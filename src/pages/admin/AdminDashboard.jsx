  ...
  return (
    <div className="w-25 h-[95vh] bg-[#0074B7] flex flex-col items-center py-7 ml-7 mr-5 rounded-3xl my-7 shadow-lg">
      <img
        ...
        alt="User"
        className="w-10 h-10 rounded-full mb-6 border-2 border-white"
      />
...
export default function NotificationDashboard() {
  ...
  return (
    <div className="flex min-h-screen">
      ...
      {/* Popup */}
      {popup && (
  <div className="fixed inset-0 bg-black/40 backdrop-blur-sm z-50 flex items-center justify-center transition-opacity duration-300">
    <div className="bg-white p-6 rounded-2xl shadow-2xl w-full max-w-md border border-gray-200">
      <div className="mb-4">
        <h2 className="text-2xl font-semibold text-gray-800 mb-1">📌 Notification Details</h2>
        <p className="text-sm text-gray-500">Here's a breakdown of the selected notification</p>
      </div>

      <div className="space-y-2">
        <div>
          <p className="text-sm font-medium text-gray-700">📝 Message:</p>
          <p className="text-gray-900">{popup.message}</p>
        </div>

        <div>
          <p className="text-sm font-medium text-gray-700">🕒 Time:</p>
          <p className="text-gray-600">{popup.time}</p>
        </div>

        <div>
          <p className="text-sm font-medium text-gray-700">📂 Type:</p>
          <p className="text-xs uppercase text-blue-600 tracking-wide">{popup.type}</p>
        </div>

        <div>
          <p className="text-sm font-medium text-gray-700">📋 Details:</p>
          <p className="text-gray-800 leading-relaxed">{popup.details}</p>
        </div>
      </div>
      ...
      <button
              ...
              className="mt-6 px-6 py-2 bg-blue-600 text-white font-medium rounded-md hover:bg-blue-700 transition-all"
            >
              Close
            </button>
      </div>
